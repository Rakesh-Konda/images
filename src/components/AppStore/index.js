...
class AppStore extends Component {
  ...
  convey = event => {
    ...
    const vv = event.target.value.toLowerCase()
    const FilteredData = appsList.filter(each =>
      each.appName.toLowerCase().includes(vv),
    )
  ...
  getData = () => {
    const {view, FilteredData} = this.state

    if (FilteredData && FilteredData.length > 0) {
      return FilteredData
    }

    const Filter = appsList.filter(each => each.category === view)
    ...
    return (
      <div className="bg">
        <div className="cen">
          ...
          <div>
            <TabItem tabsList={tabsList} Remain={this.Remain} />
          </div>
        </div>
        <div className="div">
          <AppItem AppList={FilteredData} />
        </div>
      </div>
    )
  }
